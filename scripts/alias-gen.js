...
const { normalizeUUID, improvedNormalizeString } = require('./centralized-alias-utils');
...
Object.keys(process.env).forEach(key => {
  if (key.startsWith('NOTION_') && key.endsWith('_ID')) {
    ...
    readableName = improvedNormalizeString(readableName); // Clean up readable names
    ...
    const dbAlias = `${improvedNormalizeString(readableName.toUpperCase())}_DB`; // e.g., "PROJECTS_DB", ensuring no emojis or unwanted characters

    // Map both the UUID and the readable name to the alias
    dbAliasMap[normalizedUuid] = dbAlias;                 // Normalized UUID as key
    dbAliasMap[improvedNormalizeString(readableName)] = dbAlias; // Clean readable name as key
